...
const COPY = {
  pl: {
    ...
    x: 'X',
    email: 'E-mail',
    shareText: 'Wyślij pliki w tej samej WiFi — bez chmury:',
  },
  en: {
    ...
    x: 'X',
    email: 'Email',
    shareText: 'Send files on the same WiFi — no cloud:',
  },
};
...
export default function ShareStrip({ lang }: Props) {
  ...
  return (
    <section className="share-strip" aria-label={t.shareLabel}>
      <div className="share-strip-head">
        <span className="share-strip-label">{t.shareLabel}</span>
        {visits !== null && (
          <span className="share-visit-count">
            {formatCount(visits, lang)} {t.visits}
          </span>
        )}
      </div>
      <div className="share-buttons">
        {canNativeShare && (
          <button type="button" className="share-btn share-btn-native" onClick={handleNativeShare}>
            <IconShare />
        ...
        {links.map((item) => (
          ...
          >
            {item.icon}
            <span className="share-btn-text">{item.label}</span>
          </a>
        ))}
        <button
          type="button"
